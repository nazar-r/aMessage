...
import type { RemovedMessagePayload, E2EEPeerPublicKeyPayload, NewMessagePayload, MessagesHistoryPayload, RoomConfig, MessagesData, SendMessageVariables } from "../src.a.tsx/tsx.extensions/types";
...
export const useOneOnOneRoom = ({ peerWsId }: RoomConfig) => {
  ...
  const removePendingTempId = (tempId: string) => {
    pendingOwnMessageIdsRef.current =
      pendingOwnMessageIdsRef.current.filter(
        (id) => id !== tempId
      );
  };
  ...
  const rehydrateMessages = () => {
    ...
    setMessagesCache((prev) =>
      prev.map((msg) => {
        const raw =
          encryptedTextByMessageIdRef.current.get(msg.messageId)
          ?? msg.content;
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async ({
      ...
      const encrypted = encryptRoomText(
        trimmed,
        sharedKey
      );
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const storedPeerPublicKey =
        await getStoredPeerPublicKey(peerWsId);
      ...
      const handlePeerPublicKey = async (
        payload: E2EEPeerPublicKeyPayload
      ) => {
        if (
          payload.userId !== peerWsId
          || !payload.publicKey
        ) {
          return;
        }
        ...
        const peerPublicKey = importPublicKey(
          payload.publicKey
        );

        await setStoredPeerPublicKey(
          peerWsId,
          peerPublicKey
        );
      ...
      const handleMessagesHistory = ({
        ...
        const currentMessages =
          queryClient.getQueryData<MessagesData[]>(
            messagesKey
          ) ?? [];

        const optimisticOwnMessages =
      ...
      const handleMessageRemoved = ({
        ...
        encryptedTextByMessageIdRef.current.delete(
          messageId
        );
  ...
  const updateMessage = (
    ...
    if (!sharedKey) {
      console.error(
        "E2EE shared key is not ready yet."
      );
